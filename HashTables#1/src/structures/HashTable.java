...
public class HashTable<T> implements ICollection<T> {
    ...
    private int size; // Number of elements in strcuture
	...
	public T next() {
		...
		current = current.next;
		return value;
	    } else if (index == buckets.length - 1) { // End of list
		current = null;
		return value;
	    }
	    ...
	    // Traverses until it finds the next element or until it reaches the end of the
	    // structure.
	    while (current == null && index != buckets.length - 1) {
		current = (Node) ((index == buckets.length - 1) ? null : buckets[++index]);
	    }
	    return value;
	}
